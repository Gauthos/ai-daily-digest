...
function extractAtomLink(links: unknown): string {
  ...
  const alternate = links.find((l: Record<string, unknown>) =>
    l['@_rel'] === 'alternate' || !l['@_rel']
  ) as Record<string, unknown> | undefined;

  return alternate ? String(alternate['@_href'] || '') : String((links[0] as Record<string, unknown>)?.['@_href'] || '');
}

function parseAtomEntries(entries: Record<string, unknown>[]): RawItem[] {
  return entries.map(entry => {
    ...
    return { title, link, pubDate, description: description.slice(0, 500) };
  }).filter(item => item.title || item.link);
}

function parseRSSItems(items: Record<string, unknown>[]): RawItem[] {
...
function parseFeedXml(xml: string): RawItem[] {
  ...
  if (doc.feed?.entry) {
    return parseAtomEntries(doc.feed.entry);
  }

  const channel = doc.rss?.channel || doc['rdf:RDF']?.channel;
  if (channel?.item) {
    return parseRSSItems(channel.item);
  }

  const rdfItems = doc['rdf:RDF']?.item;
  if (rdfItems) {
    return parseRSSItems(Array.isArray(rdfItems) ? rdfItems : [rdfItems]);
  }
